...
export default function FlowBuilder() {
  ...
  function onCanvasMouseMove(e) {
    ...
    // Convert viewport → content coordinates (SVG & nodes live in content space).
    ...
    d.raf = requestAnimationFrame(() => {
      ...
      const x = Math.max(0, d.lastClientX - rect.left + sl - d.offsetX);
      const y = Math.max(0, d.lastClientY - rect.top  + st - d.offsetY);
      d.lastX = x; d.lastY = y;
  ...
  function onCanvasMouseUp(e) {
    ...
    if (d) {
      ...
      const moved = finalX !== d.startX || finalY !== d.startY;
      const el = nodeRefs.current.get(d.nodeId);

      if (moved) {
        // Pre-set the inline transform to the SNAPPED value so the node doesn't
        // jitter between the unsnapped drag end and the committed position
        // during the pending re-render.
        if (el) el.style.transform = `translate3d(${finalX}px, ${finalY}px, 0)`;
        setFlow((f) => ({
          ...f,
          nodes: f.nodes.map((n) => n.id === d.nodeId ? { ...n, x: finalX, y: finalY } : n),
        }));
      }
      // Clear the perf hint only; leave transform alone. If we wipe it on a
      // pure click (no movement), React's diff is a no-op and the node snaps
      // to (0,0) with no inline transform.
  ...
  function onCanvasDrop(e) {
    ...
    const canvas = canvasRef.current;
    const r = canvas.getBoundingClientRect();
    const x = snap(e.clientX - r.left + canvas.scrollLeft - NODE_W / 2);
    const y = snap(e.clientY - r.top  + canvas.scrollTop  - NODE_H / 2);
  ...
  function startDragNode(e, nodeId) {
    ...
    dragRef.current = {
      nodeId,
      offsetX: e.clientX - r.left + canvas.scrollLeft - node.x,
      offsetY: e.clientY - r.top  + canvas.scrollTop  - node.y,
  ...
  async function save() {
    ...
    try {
      const res = await leadFlowApi.update(flow.id, {
  ...
  async function toggleStatus() {
    ...
    setSaving(true);
    try {
      await leadFlowApi.update(flow.id, { ...flow, status: nextStatus });
      setSavedAt(new Date());
    } catch (err) { setError(err.message); }
    finally { setSaving(false); }
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 130px)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, gap: 10, flexWrap: "wrap" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button className="btn btn-ghost" onClick={() => navigate("/leads/automation")}>
            <FiArrowLeft /> Back
          </button>
          <input
            value={flow.name}
...
// Multi-select for picking which channel(s) a "Send message" action uses.
function ChannelPicker({ value = [], onChange }) {
  const has = (c) => value.includes(c);
  function toggle(c) {
    onChange(has(c) ? value.filter((x) => x !== c) : [...value, c]);
  }
  const CH = [
    { key: "email",    label: "Email",    Icon: FiMail,          color: "#ec4899" },
    { key: "whatsapp", label: "WhatsApp", Icon: FiMessageSquare, color: "#25d366" },
  ];
  return (
    <div style={{ display: "flex", gap: 8 }}>
      {CH.map((c) => {
        ...
        return (
          <button
            ...
            style={{
              ...
              background: on ? `${c.color}14` : "white",
