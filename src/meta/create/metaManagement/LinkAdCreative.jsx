...
import { FiLink, FiImage, FiArrowLeft, FiArrowRight, FiFileText, FiUpload, FiX, FiGlobe, FiThumbsUp, FiMessageCircle, FiShare2, FiMoreHorizontal, FiHeart, FiSend, FiBookmark } from "react-icons/fi";
...
export default function LinkAdCreative() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.link_url.trim()) { alert("Please enter Link URL"); return; }
    ...
    try {
      ...
      navigate("/meta/create/link/launch", {
        state: { ...previousData, creative_id: response.data.id, credentials: previousData.credentials },
      });
    } catch (error) {
  ...
  let previewDomain = "";
  try { previewDomain = formData.link_url ? new URL(formData.link_url).hostname.replace(/^www\./, "") : ""; } catch { previewDomain = formData.link_url; }
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sm:p-8">
            <div className="flex items-start justify-between gap-4 mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">Create Ad Creative — Website Campaign</h1>
                  <p className="text-gray-500 text-sm mt-0.5">Create your ad creative for your website</p>
                </div>
              </div>
              <button
                onClick={() => navigate("/meta/create/link/adset", { state: previousData })}
            ...
            {previousData.campaign_id && (
              <div className="bg-gray-50 border border-gray-100 rounded-xl p-4 mb-6">
                ...
                <div className="text-sm text-gray-600 space-y-1">
                  ...
                  {previousData.adset_id && <div><strong>Ad Set ID:</strong> {previousData.adset_id || "N/A"}</div>}
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Page ID <span className="text-red-500">*</span></label>
                  {loadingPages ? (
                    ...
                    <select name="page_id" value={formData.page_id} onChange={handleInputChange} required
                      className="w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50/60 focus:bg-white focus:ring-2 focus:ring-pink-500 focus:border-transparent">
                ...
                <div>
                  ...
                  <div className="mb-4 flex gap-2">
                    <button type="button" onClick={() => handleMediaTypeChange("image")} className={`px-4 py-2 rounded-xl font-semibold text-sm transition-colors ${mediaType === "image" ? "bg-pink-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>Image</button>
                    <button type="button" onClick={() => handleMediaTypeChange("video")} className={`px-4 py-2 rounded-xl font-semibold text-sm transition-colors ${mediaType === "video" ? "bg-pink-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}>Video</button>
                  </div>
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2 flex items-center gap-2"><FiLink /> Link URL <span className="text-red-500">*</span></label>
                  <input type="url" name="link_url" value={formData.link_url} onChange={handleInputChange} placeholder="https://yourwebsite.com" required
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50/60 focus:bg-white focus:ring-2 focus:ring-pink-500 focus:border-transparent" />
                </div>
                ...
                <div>
                  ...
                  <textarea name="primary_text" value={formData.primary_text} onChange={handleInputChange} placeholder="Enter primary text for your ad" rows={4} required
                    className="w-full px-4 py-2.5 border border-gray-200 rounded-xl bg-gray-50/60 focus:bg-white focus:ring-2 focus:ring-pink-500 focus:border-transparent resize-y" />
                ...
                {placements.map((p, i) => {
                  ...
                  return (
                    <button key={p.id} type="button" onClick={() => setActivePlacement(i)}
                      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${on ? "bg-pink-500 border-pink-500 text-white shadow shadow-pink-500/30" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
            ...
            {placement.format === "feed" && (
              <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
                <div className="flex items-center gap-2.5 p-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-pink-400 to-pink-600 grid place-items-center text-white font-bold flex-shrink-0">{pageInitial}</div>
                ...
                {placement.platform === "instagram" ? (
                  <>
                    ...
                    <div className="px-3 py-2">
                      <button type="button" className="w-full inline-flex items-center justify-center gap-1.5 py-2 rounded-lg bg-gray-100 text-gray-800 text-sm font-semibold">Learn More <FiArrowRight className="w-3.5 h-3.5" /></button>
                  ...
                  <>
                    <div className="flex items-center gap-3 bg-gray-50 border-y border-gray-100 px-3 py-2.5">
                      <div className="min-w-0 flex-1">
                        <div className="text-[11px] text-gray-500 uppercase truncate">{previewDomain || "yourwebsite.com"}</div>
                      ...
                      <button type="button" className="flex-shrink-0 inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-200 text-gray-800 text-sm font-semibold">Learn More</button>
                    </div>
            ...
            {(placement.format === "story" || placement.format === "reels") && (
              <div className="relative mx-auto rounded-2xl overflow-hidden shadow-xl border border-gray-200 bg-gray-900" style={{ aspectRatio: "9 / 16", maxWidth: 300 }}>
                ...
                <div className="absolute top-0 inset-x-0 p-3 bg-gradient-to-b from-black/50 to-transparent flex items-center gap-2">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-pink-400 to-pink-600 grid place-items-center text-white text-xs font-bold ring-2 ring-white/70">{pageInitial}</div>
                ...
                <div className="absolute bottom-0 inset-x-0 p-3 bg-gradient-to-t from-black/60 to-transparent">
                  <button type="button" className="w-full inline-flex items-center justify-center gap-2 py-2.5 rounded-xl bg-white/95 text-gray-900 text-sm font-bold">Learn More <FiArrowRight className="w-4 h-4" /></button>
                  {(formData.headline || previewDomain) && (<div className="text-center text-white/90 text-[11px] mt-1.5 drop-shadow truncate">{formData.headline || previewDomain}</div>)}
                </div>
              </div>
            )}
