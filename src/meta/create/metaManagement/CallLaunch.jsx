...
import { FiSend, FiArrowLeft, FiTag, FiLayout, FiShield, FiLock, FiExternalLink } from "react-icons/fi";
import { FaBullhorn } from "react-icons/fa";
import Confetti from "react-confetti";
import metaApi from "../lcmMetaApi";

// Small rocket-on-clouds illustration for the "ready to launch" banner.
  ...
  return (
    <div className="pointer-events-none select-none absolute right-3 bottom-0 w-40 h-24">
      <svg viewBox="0 0 48 60" className="absolute right-16 bottom-7 w-11 h-14 drop-shadow-md" style={{ animation: "ln-float 3s ease-in-out infinite" }}>
        ...
        <path d="M33 50c3 6 9 6 9 6s-3-7-6-9z" fill="#fdba74" opacity="0" />
        ...
        <path d="M16 36l-6 11 6-5z" fill="#f97316" />
        <path d="M32 36l6 11-6-5z" fill="#f97316" />
...
export default function CallLaunch() {
  ...
  const handleGoHome = () => navigate("/meta/create");
      ...
      <div className="py-4 px-4 sm:px-6 max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 text-center">
          <div className="relative w-28 h-28 mx-auto mb-5">
            <span className="absolute inset-2 rounded-full bg-emerald-300" style={{ animation: "ln-ring 1.6s ease-out .3s infinite" }} />
            <div className="absolute inset-0 rounded-full bg-gradient-to-br from-green-100 to-emerald-50" />
            <div className="absolute inset-0 grid place-items-center">
              <div className="w-16 h-16 rounded-full bg-gradient-to-br from-green-400 to-emerald-600 grid place-items-center shadow-lg shadow-green-500/40" style={{ animation: "ln-pop .55s cubic-bezier(.18,1.25,.4,1) both" }}>
          ...
          <p className="text-gray-500 mb-6" style={{ animation: "ln-rise .5s ease-out .45s both" }}>Your call ad is live and will start running shortly.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-left mb-6">
            ...
            <SummaryCard icon={<FiSend className="w-4 h-4" />} color="#f97316" label="Ad ID" value={adId} />
          </div>

          <button
            onClick={handleGoHome}
            className="px-7 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 text-white font-semibold text-sm shadow-lg shadow-orange-500/30 hover:-translate-y-0.5 hover:shadow-xl transition-all inline-flex items-center gap-2"
          >
  ...
  return (
    <div className="py-4 px-4 sm:px-6 max-w-4xl mx-auto">
      ...
      <button
        onClick={() => navigate("/meta/create/call/creative", { state: previousData })}
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 sm:p-8">
        ...
        <div className="flex items-center gap-5 mb-7">
          <div className="relative flex-shrink-0">
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-orange-100 to-orange-50/60 grid place-items-center">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-orange-400 to-orange-600 grid place-items-center shadow-lg shadow-orange-500/30 -rotate-[18deg]">
                <FiSend className="w-7 h-7 text-white" />
              </div>
            </div>
            <span className="absolute -top-0.5 left-1 text-purple-400 text-lg">✦</span>
            <span className="absolute bottom-1 -right-0.5 text-emerald-400 text-sm">✦</span>
          </div>
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Launch Your Call Ad</h1>
            <p className="text-gray-500 mt-1">You're all set! Review the summary below and launch your ad.</p>
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-50 to-indigo-50/40 p-5 mb-6">
          <div className="relative z-10 flex items-start gap-3 sm:max-w-[68%]">
            <span className="w-10 h-10 rounded-xl bg-blue-100 text-blue-600 grid place-items-center flex-shrink-0">
              <FiShield className="w-5 h-5" />
            </span>
            <div>
              <div className="font-bold text-blue-700">Your ad is ready to be created.</div>
              <div className="text-sm text-gray-600 mt-0.5">
                Once launched, it will be in <strong className="text-blue-700">ACTIVE</strong> status and start running immediately.
              </div>
            </div>
          </div>
          <RocketClouds />
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap pt-1">
          <div className="flex items-center gap-2 text-xs text-gray-500">
            ...
            <span>
              By launching this ad, you agree to our{" "}
              <a href="https://www.facebook.com/policies/ads" target="_blank" rel="noopener noreferrer" className="text-purple-600 font-semibold inline-flex items-center gap-1 hover:underline">
                Advertising Policies <FiExternalLink className="w-3 h-3" />
              </a>
            </span>
          </div>
          <button
            ...
            disabled={loading}
            className="px-7 py-3 rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 text-white font-semibold text-sm shadow-lg shadow-orange-500/30 hover:-translate-y-0.5 hover:shadow-xl transition-all disabled:opacity-60 disabled:cursor-not-allowed inline-flex items-center gap-2"
          >
            {loading ? (
              <><span className="animate-spin">⏳</span> Launching Ad…</>
            ) : (
              <><FiSend className="w-4 h-4" /> Launch Ad</>
            )}
          </button>
        </div>
      </div>
