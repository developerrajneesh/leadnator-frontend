...
export default function WhatsAppAdCreative() {
  ...
  const [videoThumbnail, setVideoThumbnail] = useState(null); // For video thumbnail image
  ...
  const handleVideoSelect = async (e) => {
    ...
    setVideoId(null); // Clear previous video ID

    // Create preview instantly using object URL (non-blocking, no conversion needed)
    ...
    // Use requestIdleCallback or setTimeout to yield control to browser
    const uploadVideo = async () => {
      try {
        ...
        // Convert file to base64 in chunks to prevent blocking
        const base64String = await new Promise((resolve, reject) => {
          const reader = new FileReader();
          reader.onload = () => resolve(reader.result);
          reader.onerror = reject;
          
          // Use readAsDataURL which is async and won't block if we yield control
          reader.readAsDataURL(file);
        });
    ...
    // Yield control to browser before starting heavy operation
  ...
  const handleRemoveVideo = () => {
    ...
    setVideoPreview(null);
    setVideoId(null);
    // Also remove thumbnail when video is removed
  ...
  const handleMediaTypeChange = (type) => {
    setMediaType(type);
    // Clear existing media when switching
    if (type === "image") {
      handleRemoveVideo();
    } else {
      handleRemoveImage();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const creativePayload = {
        ...
        description: formData.description || "", // Always include description
      };
      ...
      if (mediaType === "image") {
        ...
        // Add image_hash for video thumbnail (required by Meta API)
      ...
      console.log('📤 Description in payload:', creativePayload.description);

      const response = await metaApi.createWhatsAppAdCreative(creativePayload);

      navigate("/meta/create/whatsapp/launch", {
        state: {
          ...previousData,
  ...
  return (
    <div className="space-y-6">
      <button
        onClick={() => navigate("/meta/create/whatsapp/adset", { state: previousData })}
